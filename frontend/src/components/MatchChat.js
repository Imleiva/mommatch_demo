...
import chatMessagesData from "../mock-data/chat_messages.json";
...
const MatchChat = ({ match, onClose }) => {
  ...
  const fetchMessages = useCallback(
    async (isBackground = false) => {
      ...
      if (config.useMocks) {
        // Simular mensajes del chat actual
        const mockMessages = chatMessagesData
          .filter(msg => 
            (msg.sender_id === 1 && msg.receiver_id === match.id) ||
            (msg.sender_id === match.id && msg.receiver_id === 1)
          )
          .map(msg => ({
            ...msg,
            is_read: 1 // marcar como leídos en demo
          }));

        const otherUserData = usersData.find(u => u.id === match.id);
  ...
  const sendMessage = async (e) => {
    ...
    // Modo demo: simular envío de mensaje
    if (config.useMocks) {
      setTimeout(() => {
        setMessages((prevMessages) =>
          prevMessages.map((msg) =>
            msg.id === tempId
              ? { ...msg, status: "sent", id: Date.now() }
              : msg
          )
        );
      }, 500);
  ...
  const formatPhotoUrl = (photoPath) => {
    // Modo demo: usar rutas del PUBLIC_URL
    if (config.useMocks) {
      if (!photoPath) {
        return `${process.env.PUBLIC_URL}/uploads/profiles/default_profile.jpg`;
      }
      
      // Extraer nombre del archivo y construir ruta demo
      const fileName = photoPath.split('/').pop();
      return `${process.env.PUBLIC_URL}/uploads/profiles/${fileName}`;
    }
