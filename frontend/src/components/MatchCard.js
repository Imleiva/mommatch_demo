...
const formatPhotoUrl = (photoPath) => {
  // En modo mock, usar PUBLIC_URL
  if (config.useMocks) {
    if (!photoPath) {
      return `${process.env.PUBLIC_URL}/uploads/profiles/default_profile.jpg`;
    }

    // Si ya tiene el PUBLIC_URL, devolverlo tal cual
    if (photoPath.includes(process.env.PUBLIC_URL)) {
      return photoPath;
    }

    // Si empieza con /public/, convertir a la ruta correcta
    if (photoPath.startsWith("/public/uploads/profiles/")) {
      return `${process.env.PUBLIC_URL}${photoPath.replace("/public", "")}`;
    }
    ...
    <div className="match-list">
      {loading ? (
        ...
        <div className="match-carousel">
          ...
          <MatchCard
            profile={{
              id: currentProfile.id || currentProfile.user_id || "unknown",
