...
import matchPreferencesData from "../mock-data/match_preferences.json";
...
// En GitHub Pages, usamos mocks y no hacemos llamadas al backend
...
const MatchPage = () => {
  ...
  const fetchMatchPreferences = async () => {
    ...
    if (config.useMocks) {
      console.log("Demo mode: skipping fetchMatchPreferences");
      return;
    }
  ...
  useEffect(() => {
    const fetchMatchesData = async () => {
      // Load mock data on GitHub Pages (demo mode)
      if (config.useMocks) {
        console.log("Demo mode: loading mock data for matches");

        // Cargar perfiles mock (excluir el usuario actual - ID 1)
        const mockProfiles = profilePreferencesData
          .filter((p) => p.user_id !== 1) // Excluir usuario actual
          .map((profile) => {
          ...
          .slice(0, 10); // Limitar a 10 perfiles para la demo

        // Cargar matches confirmados (matched status)
        const confirmedMatches = matchesData
          .filter(
            (m) =>
              m.status === "matched" && (m.user_id_1 === 1 || m.user_id_2 === 1)
          )
          .map((match) => {
            const otherUserId =
              match.user_id_1 === 1 ? match.user_id_2 : match.user_id_1;
            const profile = profilePreferencesData.find(
              (p) => p.user_id === otherUserId
            );
            const user = usersData.find((u) => u.id === otherUserId);
        ...
        // Cargar likes pendientes (pending status donde user_id_1 es 1)
        const pendingLikes = matchesData
          .filter((m) => m.status === "pending" && m.user_id_1 === 1)
          .map((match) => {
            const profile = profilePreferencesData.find(
              (p) => p.user_id === match.user_id_2
            );
            const user = usersData.find((u) => u.id === match.user_id_2);
            if (!profile || !user) return null;
            return {
        ...
        // Cargar rechazados (rejected status donde user_id_1 es 1)
        const rejectedProfiles = matchesData
          .filter((m) => m.status === "rejected" && m.user_id_1 === 1)
          .map((match) => {
            const profile = profilePreferencesData.find(
              (p) => p.user_id === match.user_id_2
            );
        ...
        setLikedProfiles(pendingLikes);
        setRejectedProfiles(rejectedProfiles);
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    const verifySession = async () => {
      // Skip API calls on GitHub Pages (demo mode)
      if (config.useMocks) {
        console.log("Demo mode: skipping session verification");
        return;
      }
